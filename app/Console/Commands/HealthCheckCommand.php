...
class HealthCheckCommand extends Command
{
    ...
    protected function checkTimeZone(): HealthCheckStateEnum
    {
        ...
        if ($dbDateTime !== $appDateTime) {
            return tap(
                ...
                function (HealthCheckStateEnum $state) use ($appTimezone, $dbTimeZone) {
    ...
    protected function checkPhpExtensions(): HealthCheckStateEnum
    {
        ...
        if ($missingExtensions->isNotEmpty()) {
            return tap(
                HealthCheckStateEnum::FAILING(),
                function (HealthCheckStateEnum $state) use ($missingExtensions) {
    ...
    protected function checkMemoryLimit(int $limit = 128): HealthCheckStateEnum
    {
        ...
        if ($localValue < $limit) {
            return tap(
                HealthCheckStateEnum::FAILING(),
                function (HealthCheckStateEnum $state) use ($limit, $localValue) {
